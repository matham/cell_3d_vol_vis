...
from cell_3d_vol_vis.main import main, CellsData
...
class Worker(WorkerBase):
    ...
    def work(self) -> tuple[tuple[int, int, int], list[list[Cell]], list[types.array]]:
...
class VolumeVisWidget:
    ...
    def __init__(self):
        self.image_layers: list[napari.layers.Image] = []
        self.viewer: napari.Viewer | None = None
        self.cell_layers: list[napari.layers.Points] = []
        self.seg_layer: napari.layers.Image | None = None

        self.worker: Worker | None = None

        self.run_widget: FunctionGui | None = None
        self.image_layers_widget: FunctionGui | None = None
        self.cell_layers_widget: FunctionGui | None = None
    ...
    def process_worker_result(
        ...
        for cell_layer in self.cell_layers:
            cell_layer.visible = False
        if self.seg_layer is not None:
            self.seg_layer.visible = False

        for image, image_layer in zip(images, self.image_layers):
            ...
            if update_existing_layers and name in self.viewer.layers:
                ...
                new_layer = self.viewer.add_image(
                    data=image, name=name, translate=cuboid_offset,
                )
                for prop in (
                        "opacity", "colormap", "contrast_limits", "contrast_limits_range", "gamma", "attenuation",
                        "rendering", "blending",
                ):
                    setattr(new_layer, prop, getattr(image_layer, prop))

            if scale_to_voxel:
                new_layer.translate = [o * v for o, v in zip(cuboid_offset, voxel_size)]
                new_layer.scale = voxel_size

        for layer_cells, cell_layer in zip(cells, self.cell_layers):
            # cell_layer.scale = voxel_size
            ...
            if update_existing_layers and name in self.viewer.layers:
                ...
                for prop in (
                        "opacity", "symbol", "face_color", "size", "text", "border_width", "border_color",
                        "border_colormap", "border_contrast_limits", "face_colormap", "face_contrast_limits",
                        "out_of_slice_display", "shading", "antialiasing", "canvas_size_limits",
                ):
    ...
    def view_2d(self):
        ...
        center = int(viewer.dims.current_step[0]), int(viewer.camera.center[1]), int(viewer.camera.center[2])
        if do_scale:
    ...
    def image_layers_run(
        ...
        Parameters
        ----------
        image_layers : napari.layers.Image
    ...
    def cell_layers_run(
        self,
        cell_layers: list[napari.layers.Points],
    ):
        """
        ...
        Parameters
        ----------
        cell_layers : napari.layers.Points
    ...
    def segmentation_layer_run(
        self,
        seg_layer: napari.layers.Image | None,
    ):
        """
        magicgui widget for setting the cell layer.
    ...
    def run(
        ...
        cell_layers: list[napari.layers.Points] = self.cell_layers
        ...
        center = int(viewer.dims.current_step[0]), int(viewer.camera.center[1]), int(viewer.camera.center[2])
        if scale_to_voxel:
            center = center[0], int(center[1] / voxel_size[1]), int(center[2] / voxel_size[2])
        cuboid_size_px = [int(round(size / um_ppx)) for size, um_ppx in zip(cuboid_size, voxel_size)]
        # todo: handle outside range
        offset = [c - size // 2 for c, size in zip(center, cuboid_size_px)]

        worker = Worker(
            images=[layer.data for layer in image_layers],
            cells=cells,
            cuboid_offset=offset,
            cuboid_size=cuboid_size_px,
            region_segmentation=None if self.seg_layer is None else self.seg_layer.data,
            selected_region_id=None if selected_region_id < 0 else selected_region_id,
        ...
        worker.returned.connect(
            partial(
                self.process_worker_result, voxel_size=voxel_size, show_in_3d=show_in_3d,
                scale_to_voxel=scale_to_voxel, update_existing_layers=update_existing_layers,
            )
        )
    ...
    def build(self) -> widgets.Container:
        self.run_widget = FunctionGui(self.run,
            call_button=True,
            persist=True,
            param_options={
        ...
        self.image_layers_widget = FunctionGui(
            self.image_layers_run,
            call_button=False,
            persist=False,
            scrollable=False,
            labels=False,
            auto_call=True,
            param_options={"image_layers": {"layout": "vertical"}},
        )
        self.cell_layers_widget = FunctionGui(
            self.cell_layers_run,
            call_button=False,
            persist=False,
            scrollable=False,
            labels=False,
            auto_call=True,
            param_options={"cell_layers": {"layout": "vertical"}},
        )
        ...
        for widget, new_name, insertion in zip(
                (self.image_layers_widget, self.cell_layers_widget, self.segmentation_layer_widget),
                ("Image layers", "Cell layers", "Segmentation layer"),
                ("selected_cells_only", "voxel_size", "region_cuboids_path"),
                strict=True,
        ):
        ...
        container = widgets.Container(
            widgets=[
                run_widget,
                FunctionGui(self.set_layers_voxel_sizes, call_button="Set layers voxel size", auto_call=False),
                FunctionGui(self.view_2d, call_button="Show in 2D", auto_call=False),
                FunctionGui(self.view_3d, call_button="Show in 3D", auto_call=False),
            ],
            layout="vertical",
            labels=False,
        )
